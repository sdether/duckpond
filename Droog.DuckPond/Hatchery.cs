...
#if DEBUG
                _moduleBuilder = _assemblyBuilder.DefineDynamicModule(name, string.Format("{0}.mod", name), true);
#else
            ...
            public MethodMap[] GetMap(Type instanceType, Type interfaceType) {
                ...
                foreach(var interfaceMethod in interfaceMethods) {
                    ...
                    var instanceMethod = (from method in instanceMethods
                                          where method.Name == interfaceMethod.Name
                                                && method.IsGenericMethod == interfaceMethod.IsGenericMethod
                                                && HasSameParameters(method, parameterInfo)
                                          select method).FirstOrDefault();
                    if(instanceMethod == null) {
                        return null;
                    }
                    maps.Add(new MethodMap(instanceMethod,interfaceMethod,parameterInfo.Select(x => x.ParameterType).ToArray()));
                }
                return maps.ToArray();
            }
